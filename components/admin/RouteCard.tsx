      ...
      <div className="border-t border-gray-100 pt-4">
        ...
        <div className="mt-4 border-t border-gray-200 pt-4">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
            {/* Toolbar */}
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-blue-200 p-2.5 sm:p-3 md:p-4">
              <div className="flex flex-col gap-3 sm:gap-4">
                {/* Title Section */}
                <div className="flex items-start sm:items-center gap-2 sm:gap-3 min-w-0">
                  <div className="w-9 h-9 sm:w-11 sm:h-11 md:w-12 md:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 text-white flex items-center justify-center shadow-lg flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2 flex-wrap">
                      <h4 className="text-xs sm:text-sm md:text-base lg:text-lg font-extrabold text-gray-900 leading-tight">
                        {fixedTripType === 'arrival'
                          ? 'رحلات القادمين'
                      ...
                      <span className="text-[10px] sm:text-[11px] font-extrabold px-1.5 sm:px-2 py-0.5 rounded-full bg-white border border-blue-200 text-blue-800 whitespace-nowrap">
                        {visibleTrips.length} رحلة
                      </span>
                    </div>
                    <p className="text-[10px] sm:text-[11px] md:text-xs text-gray-600 font-semibold mt-1 line-clamp-2 sm:truncate">
                ...
                <div className="flex flex-col gap-2 sm:gap-2.5">
                  {/* Segmented filter */}
                  <div className="inline-flex items-center bg-white border border-gray-200 rounded-xl sm:rounded-2xl p-0.5 sm:p-1 w-full sm:w-auto overflow-x-auto">
                    <button
                      type="button"
                      onClick={() => onTripListFilterChange('upcoming')}
                      className={`px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-[10px] sm:text-xs md:text-sm font-extrabold inline-flex items-center gap-1.5 sm:gap-2 transition whitespace-nowrap flex-shrink-0 ${
                    ...
                    >
                      ...
                      <span className="hidden sm:inline">المنتهية</span>
                      <span className="sm:hidden">منتهية</span>
                    </button>
                    <button
                      type="button"
                  ...
                  <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
                    <button
                      ...
                      className={`px-2 sm:px-3 py-1.5 sm:py-2 rounded-xl sm:rounded-2xl border font-extrabold text-[10px] sm:text-xs md:text-sm inline-flex items-center justify-center gap-1.5 sm:gap-2 transition ${
                    ...
                    >
                      {expandedRouteTrips[route.id] ? <EyeOff className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> : <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
                      <span className="hidden sm:inline">{expandedRouteTrips[route.id] ? 'إخفاء' : 'عرض'}</span>
                    </button>
                    ...
                    <button
                      ...
                      className="px-2 sm:px-3 py-1.5 sm:py-2 rounded-xl sm:rounded-2xl border border-gray-200 bg-white text-gray-900 hover:bg-gray-50 font-extrabold text-[10px] sm:text-xs md:text-sm inline-flex items-center justify-center gap-1.5 sm:gap-2 disabled:opacity-50 transition"
                      title="تحديث الرحلات"
                    >
                      <RefreshCcw className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${routeTripsLoading[route.id] ? 'animate-spin' : ''}`} />
                    ...
                    <button
                      ...
                      className="px-2 sm:px-3 py-1.5 sm:py-2 rounded-xl sm:rounded-2xl border border-emerald-200 bg-emerald-600 text-white hover:bg-emerald-700 font-extrabold text-[10px] sm:text-xs md:text-sm inline-flex items-center justify-center gap-1.5 sm:gap-2 transition"
                      title="نسخ الكشف"
                    >
