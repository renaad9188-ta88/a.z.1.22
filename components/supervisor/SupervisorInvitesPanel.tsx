...
import { Phone, MessageCircle, Upload, Search, CheckCircle2, UserPlus, RefreshCw, Plus, Pencil, X } from 'lucide-react'
...
interface SupervisorInvitesPanelProps {
  supervisorId: string
}

export default function SupervisorInvitesPanel({ supervisorId }: SupervisorInvitesPanelProps) {
  ...
  const load = async () => {
    try {
      setLoading(true)
      // جلب دعوات المشرف من supervisor_invites
      const { data: supervisorInvites, error: e1 } = await supabase
      ...
      const inviteIds = (supervisorInvites || []).map((si: any) => si.invite_id)
      ...
      console.error('SupervisorInvitesPanel load error:', e)
  ...
  const linkInviteToSupervisor = async (inviteId: string) => {
    try {
      ...
      console.error('Link invite to supervisor error:', e)
  ...
  const addOne = async () => {
    ...
    try {
      ...
      const { data: newInvite, error } = await supabase
        .from('invites')
        .upsert(payload as any, { onConflict: 'phone' })
        .select('id')
        .single()
      ...
      // ربط الدعوة بالمشرف
      await linkInviteToSupervisor(newInvite.id)
  ...
  const doImport = async () => {
    ...
    try {
      ...
      // ربط جميع الدعوات الجديدة بالمشرف
      if (newInvites && newInvites.length > 0) {
        for (const invite of newInvites) {
          await linkInviteToSupervisor(invite.id)
        }
      }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
      ...
      <div className="bg-blue-50 rounded-lg p-4 border border-blue-100 mb-4">
        ...
        {/* Message template */}
        <div className="mt-3 border border-gray-200 rounded-xl p-3 sm:p-4 bg-white">
          <div className="text-xs sm:text-sm font-extrabold text-gray-900 flex items-center gap-2">
