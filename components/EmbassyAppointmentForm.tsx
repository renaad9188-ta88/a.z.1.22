...
export default function EmbassyAppointmentForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.fullName || !formData.phone || !formData.embassyType || !passportImage) {
    ...
    try {
      ...
      const { data: requestData, error } = await supabase
        ...
        .insert({
          ...
          city: `مقابلة السفارة: ${formData.embassyType}`,
          passport_image_url: passportImageUrl,
          status: 'pending',
          visit_type: 'embassy',
          travel_date: formData.appointmentDate || new Date().toISOString().split('T')[0],
          days_count: 1,
          nationality: 'سوري',
          passport_number: 'N/A',
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-lg shadow-xl p-4 sm:p-6 md:p-8">
            ...
            <div className="space-y-4 sm:space-y-6 mb-6 sm:mb-8">
              ...
              <div className="border-b border-gray-200 pb-3 sm:pb-4">
                <label className="text-xs sm:text-sm font-medium text-gray-500 mb-1 block">نوع السفارة</label>
                <p className="text-base sm:text-lg md:text-xl font-semibold text-gray-800">{formData.embassyType}</p>
              </div>
              <div className="border-b border-gray-200 pb-3 sm:pb-4">
                <label className="text-xs sm:text-sm font-medium text-gray-500 mb-1 block">تاريخ الموعد</label>
                <p className="text-base sm:text-lg md:text-xl font-semibold text-gray-800">{formData.appointmentDate || 'غير محدد'}</p>
              </div>
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-lg shadow-xl p-4 sm:p-6 md:p-8">
          <div className="text-center mb-6 sm:mb-8">
            ...
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 mb-2">خدمات مقابلة السفارة</h1>
            <p className="text-sm sm:text-base text-gray-600">حجز مواعيد السفارة وتنظيم جميع المستندات المطلوبة</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div>
              ...
              <input type="tel" required value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500" placeholder="+966XXXXXXXXX" />
            </div>
            ...
            <div>
              ...
              <select required value={formData.embassyType} onChange={(e) => setFormData({ ...formData, embassyType: e.target.value })} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base text-gray-900 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500">
                ...
                <option value="سفارة أخرى">سفارة أخرى</option>
              </select>
            </div>

            <div lang="en" dir="ltr">
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">تاريخ الموعد المفضل</label>
              <input type="date" value={formData.appointmentDate} onChange={(e) => setFormData({ ...formData, appointmentDate: e.target.value })} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500" lang="en" />
            </div>

            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">الغرض من المقابلة</label>
              <textarea value={formData.purpose} onChange={(e) => setFormData({ ...formData, purpose: e.target.value })} rows={3} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 resize-none" placeholder="اذكر الغرض من المقابلة..." />
            </div>

            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">المستندات المطلوبة</label>
              <textarea value={formData.documents} onChange={(e) => setFormData({ ...formData, documents: e.target.value })} rows={3} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 resize-none" placeholder="اذكر المستندات التي تحتاجها..." />
            </div>
            ...
            <button type="submit" className="w-full py-2.5 sm:py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold text-sm sm:text-base md:text-lg">عرض الملخص</button>
          </form>
        </div>
      </div>
    </div>
